...
async function createNewUser({
  ...
  const user = await MySQL.sequelize.query(
    "INSERT INTO user (email, password, firstName, lastName, city, state, country, zipCode) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    {
...
async function changeUserPassword({ userId, oldPassword, newPassword }) {
  ...
  if (res[0].is_active || res[0].is_blocked || res[0].is_deleted) {
    const msg =
      "User is not allowed to perform any action. Account is susspended";
    const err = new Error(msg);
    err.code = 403;
    err.msg = msg;
    throw err;
...
async function changeUserEmail({ userId, oldEmail, newEmail, password }) {
  const res = await MySQL.sequelize.query("SELECT * FROM users WHERE id = ?", {
    type: MySQL.sequelize.QueryTypes.SELECT,
    replacements: [userId],
  });
